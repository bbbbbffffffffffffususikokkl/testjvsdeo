...
/**
 * Inlines proxy functions — single-expression wrapper functions — at every
 * call site, e.g.:
 *
    ...
    public execute(log: LogFunction): boolean {
        ...
        traverse(this.ast, {
            enter(path) {
                ...
                const variable = findConstantVariable<ProxyFunctionExpression>(
                    path,
                    isProxyFunctionExpression,
                    true
                );
                if (!variable) {
                    return;
                ...
                usages.push(
                    ...proxyFunction
                        .getCalls()
                        .map(p => [p, proxyFunction] as [NodePath, ProxyFunctionVariable])
                );
            }
        });

        // Replace innermost proxy calls first to avoid stale paths

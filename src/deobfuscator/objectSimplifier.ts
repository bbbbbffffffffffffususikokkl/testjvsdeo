...
/**
 * Inlines property accesses and proxy-function calls on constant object
 * literals, e.g.:
 *
 *   var obj = { a: 1, fn: function(x) { return x + 1; } };
 *   obj.a        → 1
 *   obj.fn(5)    → 5 + 1
 */
    ...
    public execute(log: LogFunction): boolean {
        ...
        traverse(this.ast, {
            enter(path) {
                ...
                const variable = findConstantVariable<ProxyObjectExpression>(
                    path,
                    isProxyObjectExpression
                );
                if (!variable) {
                    return;
                }

                // Check if object values are modified (unsafe to inline if so)
                ...
                usages.push(
                    ...proxyObject
                        .getUsages()
                        .map(p => [p, proxyObject] as [NodePath, ProxyObject])
                );
            }
        });

        // Replace innermost usages first to avoid stale paths

...
/**
 * Converts computed property accesses / object keys that use string literals
 * into dot-notation / identifier keys where possible.
 *
 * Examples:
 *   obj["foo"]  →  obj.foo
 *   { "bar": 1 }  →  { bar: 1 }
        ...
        traverse(this.ast, {
            ...
            ['ObjectProperty|ObjectMethod' as any](
                ...
                if (
                    ...
                    // Keep numeric / non-identifier string keys as computed=false with literal key
                    path.node.computed = false;
                    self.setChanged();
                }
            }
